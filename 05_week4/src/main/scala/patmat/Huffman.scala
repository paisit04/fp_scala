...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def incr(charCount:Map[Char, Int], char:Char) = {
       val count = (charCount get char).getOrElse(0) + 1
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    freqs.sortWith((fork1,fork2) => fork1._2 < fork2._2).map((f) => Leaf (f._1, f._2))
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case l :: r :: cs => (makeCodeTree(l, r) :: cs).sortWith(weight(_) < weight(_))
    case _ => trees
  }
   ...
   */
  def until(singleton_fn: List[CodeTree]=>Boolean, combine_fn: List[CodeTree]=>List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
    if (singleton_fn(trees)) trees
    else until(singleton_fn, combine_fn)( combine_fn(trees) )
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def lookup(tree:  CodeTree)(char: Char): List[Bit] = tree match {
      case Leaf(c, w) => List()
      case Fork(l, r, cs, w) if chars(l).contains(char) => 0 :: lookup(l)(char)
      case Fork(l, r, cs, w) => 1 :: lookup(r)(char)
    }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table match {
    ...
    case (x, bs) :: xs => if (x == char) bs else codeBits(xs)(char)
  }
