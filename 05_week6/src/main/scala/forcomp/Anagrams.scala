...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    ((w.toLowerCase groupBy identity) map {case (c,cs) => (c, cs.length)}).toList.sortBy(_._1)
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (for {
      (x_c, x_n) <- x
    } yield (x_c, x_n - y.toMap.getOrElse(x_c,0))
    ) sortBy (_._1) filter (_._2!=0) 
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def occurrenceAnagrams(occurrences: Occurrences) : List[Sentence] = {
      if (occurrences.isEmpty) List(List())
        ...
        for {
          co <- combinations(occurrences)
          word <- dictionaryByOccurrences getOrElse (co, Nil)
